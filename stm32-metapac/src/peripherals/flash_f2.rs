...
fn default () -> Optcr { Optcr (0) } } impl core :: fmt :: Debug for Optcr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Optcr") . field ("optlock" , & self . optlock ()) . field ("optstrt" , & self . optstrt ()) . field ("bor_lev" , & self . bor_lev ()) . field ("wdg_sw" , & self . wdg_sw ()) . field ("n_rst_stop" , & self . n_rst_stop ()) . field ("n_rst_stdby" , & self . n_rst_stdby ()) . field ("rdp" , & self . rdp ()) . field ("n_wrp" , & self . n_wrp ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Optcr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Optcr {{ optlock: {=bool:?}, optstrt: {=bool:?}, bor_lev: {=u8:?}, wdg_sw: {=bool:?}, n_rst_stop: {=bool:?}, n_rst_stdby: {=bool:?}, rdp: {=u8:?}, n_wrp: {=u16:?} }}" , self . optlock () , self . optstrt () , self . bor_lev () , self . wdg_sw () , self . n_rst_stop () , self . n_rst_stdby () , self . rdp () , self . n_wrp ()) } } # [doc = "Status register"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
