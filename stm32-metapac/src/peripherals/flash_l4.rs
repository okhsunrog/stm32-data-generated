...
fn default () -> Acr { Acr (0) } } impl core :: fmt :: Debug for Acr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Acr") . field ("latency" , & self . latency ()) . field ("prften" , & self . prften ()) . field ("icen" , & self . icen ()) . field ("dcen" , & self . dcen ()) . field ("icrst" , & self . icrst ()) . field ("dcrst" , & self . dcrst ()) . field ("run_pd" , & self . run_pd ()) . field ("sleep_pd" , & self . sleep_pd ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Acr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Acr {{ latency: {=u8:?}, prften: {=bool:?}, icen: {=bool:?}, dcen: {=bool:?}, icrst: {=bool:?}, dcrst: {=bool:?}, run_pd: {=bool:?}, sleep_pd: {=bool:?} }}" , self . latency () , self . prften () , self . icen () , self . dcen () , self . icrst () , self . dcrst () , self . run_pd () , self . sleep_pd ()) } } # [doc = "Flash control register"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
...
impl defmt :: Format for Eccr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Eccr {{ addr_ecc: {=u32:?}, bk_ecc: {=bool:?}, sysf_ecc: {=bool:?}, eccie: {=bool:?}, eccc: {=bool:?}, eccd: {=bool:?} }}" , self . addr_ecc () , self . bk_ecc () , self . sysf_ecc () , self . eccie () , self . eccc () , self . eccd ()) } } # [doc = "Flash option register"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
...
fn default () -> Optr { Optr (0) } } impl core :: fmt :: Debug for Optr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Optr") . field ("rdp" , & self . rdp ()) . field ("bor_lev" , & self . bor_lev ()) . field ("n_rst_stop" , & self . n_rst_stop ()) . field ("n_rst_stdby" , & self . n_rst_stdby ()) . field ("idwg_sw" , & self . idwg_sw ()) . field ("iwdg_stop" , & self . iwdg_stop ()) . field ("iwdg_stdby" , & self . iwdg_stdby ()) . field ("wwdg_sw" , & self . wwdg_sw ()) . field ("bfb" , & self . bfb ()) . field ("dualbank" , & self . dualbank ()) . field ("n_boot1" , & self . n_boot1 ()) . field ("sram2_pe" , & self . sram2_pe ()) . field ("sram2_rst" , & self . sram2_rst ()) . field ("n_swboot0" , & self . n_swboot0 ()) . field ("n_boot0" , & self . n_boot0 ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Optr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Optr {{ rdp: {=u8:?}, bor_lev: {=u8:?}, n_rst_stop: {=bool:?}, n_rst_stdby: {=bool:?}, idwg_sw: {=bool:?}, iwdg_stop: {=bool:?}, iwdg_stdby: {=bool:?}, wwdg_sw: {=bool:?}, bfb: {=bool:?}, dualbank: {=bool:?}, n_boot1: {=bool:?}, sram2_pe: {=bool:?}, sram2_rst: {=bool:?}, n_swboot0: {=bool:?}, n_boot0: {=bool:?} }}" , self . rdp () , self . bor_lev () , self . n_rst_stop () , self . n_rst_stdby () , self . idwg_sw () , self . iwdg_stop () , self . iwdg_stdby () , self . wwdg_sw () , self . bfb () , self . dualbank () , self . n_boot1 () , self . sram2_pe () , self . sram2_rst () , self . n_swboot0 () , self . n_boot0 ()) } } # [doc = "Flash Bank 1 PCROP End address register"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
...
impl defmt :: Format for Pcrop2sr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Pcrop2sr {{ pcrop2_strt: {=u16:?} }}" , self . pcrop2_strt ()) } } # [doc = "Status register"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
...
impl defmt :: Format for Wrp2br { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Wrp2br {{ wrp2b_strt: {=u8:?}, wrp2b_end: {=u8:?} }}" , self . wrp2b_strt () , self . wrp2b_end ()) } } }