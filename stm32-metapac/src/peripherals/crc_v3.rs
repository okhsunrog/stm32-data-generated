...
fn default () -> Cr { Cr (0) } } impl core :: fmt :: Debug for Cr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Cr") . field ("reset" , & self . reset ()) . field ("polysize" , & self . polysize ()) . field ("rev_in" , & self . rev_in ()) . field ("rev_out" , & self . rev_out ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Cr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Cr {{ reset: {=bool:?}, polysize: {:?}, rev_in: {:?}, rev_out: {:?} }}" , self . reset () , self . polysize () , self . rev_in () , self . rev_out ()) } } } pub mod vals { # [repr (u8)]
...
HALF_WORD = 0x02 , # [doc = "Bit reversal done by word"]
