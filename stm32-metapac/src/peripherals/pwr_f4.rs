...
fn default () -> Cr1 { Cr1 (0) } } impl core :: fmt :: Debug for Cr1 { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Cr1") . field ("lpds" , & self . lpds ()) . field ("pdds" , & self . pdds ()) . field ("cwuf" , & self . cwuf ()) . field ("csbf" , & self . csbf ()) . field ("pvde" , & self . pvde ()) . field ("pls" , & self . pls ()) . field ("dbp" , & self . dbp ()) . field ("fpds" , & self . fpds ()) . field ("lplvds" , & self . lplvds ()) . field ("mrlvds" , & self . mrlvds ()) . field ("adcdc1" , & self . adcdc1 ()) . field ("vos" , & self . vos ()) . field ("oden" , & self . oden ()) . field ("odswen" , & self . odswen ()) . field ("uden" , & self . uden ()) . field ("fmssr" , & self . fmssr ()) . field ("fissr" , & self . fissr ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Cr1 { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Cr1 {{ lpds: {=bool:?}, pdds: {:?}, cwuf: {=bool:?}, csbf: {=bool:?}, pvde: {=bool:?}, pls: {=u8:?}, dbp: {=bool:?}, fpds: {=bool:?}, lplvds: {=bool:?}, mrlvds: {=bool:?}, adcdc1: {=bool:?}, vos: {:?}, oden: {=bool:?}, odswen: {=bool:?}, uden: {=u8:?}, fmssr: {=bool:?}, fissr: {=bool:?} }}" , self . lpds () , self . pdds () , self . cwuf () , self . csbf () , self . pvde () , self . pls () , self . dbp () , self . fpds () , self . lplvds () , self . mrlvds () , self . adcdc1 () , self . vos () , self . oden () , self . odswen () , self . uden () , self . fmssr () , self . fissr ()) } } # [doc = "power control/status register"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
...
fn default () -> Csr1 { Csr1 (0) } } impl core :: fmt :: Debug for Csr1 { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Csr1") . field ("wuf" , & self . wuf ()) . field ("sbf" , & self . sbf ()) . field ("pvdo" , & self . pvdo ()) . field ("brr" , & self . brr ()) . field ("ewup2" , & self . ewup2 ()) . field ("ewup" , & self . ewup ()) . field ("bre" , & self . bre ()) . field ("vosrdy" , & self . vosrdy ()) . field ("odrdy" , & self . odrdy ()) . field ("odswrdy" , & self . odswrdy ()) . field ("udrdy" , & self . udrdy ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Csr1 { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Csr1 {{ wuf: {=bool:?}, sbf: {=bool:?}, pvdo: {=bool:?}, brr: {=bool:?}, ewup2: {=bool:?}, ewup: {=bool:?}, bre: {=bool:?}, vosrdy: {=bool:?}, odrdy: {=bool:?}, odswrdy: {=bool:?}, udrdy: {=u8:?} }}" , self . wuf () , self . sbf () , self . pvdo () , self . brr () , self . ewup2 () , self . ewup () , self . bre () , self . vosrdy () , self . odrdy () , self . odswrdy () , self . udrdy ()) } } } pub mod vals { # [repr (u8)]
