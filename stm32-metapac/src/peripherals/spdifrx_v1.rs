...
impl defmt :: Format for Dir { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Dir {{ thi: {=u16:?}, tlo: {=u16:?} }}" , self . thi () , self . tlo ()) } } # [doc = "Data input register"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
...
fn default () -> Dr { Dr (0) } } impl core :: fmt :: Debug for Dr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Dr") . field ("dr" , & self . dr ()) . field ("pe" , & self . pe ()) . field ("v" , & self . v ()) . field ("u" , & self . u ()) . field ("c" , & self . c ()) . field ("pt" , & self . pt ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Dr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Dr {{ dr: {=u32:?}, pe: {=bool:?}, v: {=bool:?}, u: {=bool:?}, c: {=bool:?}, pt: {=u8:?} }}" , self . dr () , self . pe () , self . v () , self . u () , self . c () , self . pt ()) } } # [doc = "Interrupt Flag Clear register"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
