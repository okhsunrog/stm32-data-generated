...
fn default () -> Ccr { Ccr (0) } } impl core :: fmt :: Debug for Ccr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Ccr") . field ("dual" , & self . dual ()) . field ("delay" , & self . delay ()) . field ("damdf" , & self . damdf ()) . field ("ckmode" , & self . ckmode ()) . field ("presc" , & self . presc ()) . field ("vrefen" , & self . vrefen ()) . field ("vsenseen" , & self . vsenseen ()) . field ("vbaten" , & self . vbaten ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Ccr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Ccr {{ dual: {:?}, delay: {=u8:?}, damdf: {:?}, ckmode: {:?}, presc: {:?}, vrefen: {=bool:?}, vsenseen: {=bool:?}, vbaten: {=bool:?} }}" , self . dual () , self . delay () , self . damdf () , self . ckmode () , self . presc () , self . vrefen () , self . vsenseen () , self . vbaten ()) } } # [doc = "ADC common regular data register for dual and triple modes"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
...
impl defmt :: Format for Cdr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Cdr {{ rdata_mst: {=u16:?}, rdata_slv: {=u16:?} }}" , self . rdata_mst () , self . rdata_slv ()) } } # [doc = "ADC x common regular data register for 32-bit dual mode"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
