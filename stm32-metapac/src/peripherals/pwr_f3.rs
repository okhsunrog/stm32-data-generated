...
fn default () -> Cr { Cr (0) } } impl core :: fmt :: Debug for Cr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Cr") . field ("lpds" , & self . lpds ()) . field ("pdds" , & self . pdds ()) . field ("cwuf" , & self . cwuf ()) . field ("csbf" , & self . csbf ()) . field ("pvde" , & self . pvde ()) . field ("pls" , & self . pls ()) . field ("dbp" , & self . dbp ()) . field ("ensd[0]" , & self . ensd (0usize)) . field ("ensd[1]" , & self . ensd (1usize)) . field ("ensd[2]" , & self . ensd (2usize)) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Cr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Cr {{ lpds: {=bool:?}, pdds: {:?}, cwuf: {=bool:?}, csbf: {=bool:?}, pvde: {=bool:?}, pls: {=u8:?}, dbp: {=bool:?}, ensd[0]: {=bool:?}, ensd[1]: {=bool:?}, ensd[2]: {=bool:?} }}" , self . lpds () , self . pdds () , self . cwuf () , self . csbf () , self . pvde () , self . pls () , self . dbp () , self . ensd (0usize) , self . ensd (1usize) , self . ensd (2usize)) } } # [doc = "power control/status register"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
...
fn default () -> Csr { Csr (0) } } impl core :: fmt :: Debug for Csr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Csr") . field ("wuf" , & self . wuf ()) . field ("sbf" , & self . sbf ()) . field ("pvdo" , & self . pvdo ()) . field ("vrefintrdyf" , & self . vrefintrdyf ()) . field ("ewup[0]" , & self . ewup (0usize)) . field ("ewup[1]" , & self . ewup (1usize)) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Csr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Csr {{ wuf: {=bool:?}, sbf: {=bool:?}, pvdo: {=bool:?}, vrefintrdyf: {=bool:?}, ewup[0]: {=bool:?}, ewup[1]: {=bool:?} }}" , self . wuf () , self . sbf () , self . pvdo () , self . vrefintrdyf () , self . ewup (0usize) , self . ewup (1usize)) } } } pub mod vals { # [repr (u8)]
