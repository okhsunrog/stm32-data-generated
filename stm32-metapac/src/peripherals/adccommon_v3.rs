...
fn default () -> Ccr { Ccr (0) } } impl core :: fmt :: Debug for Ccr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Ccr") . field ("mult" , & self . mult ()) . field ("delay" , & self . delay ()) . field ("dmacfg" , & self . dmacfg ()) . field ("mdma" , & self . mdma ()) . field ("ckmode" , & self . ckmode ()) . field ("vrefen" , & self . vrefen ()) . field ("ch18sel" , & self . ch18sel ()) . field ("ch17sel" , & self . ch17sel ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Ccr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Ccr {{ mult: {=u8:?}, delay: {=u8:?}, dmacfg: {:?}, mdma: {=u8:?}, ckmode: {=u8:?}, vrefen: {=bool:?}, ch18sel: {=bool:?}, ch17sel: {=bool:?} }}" , self . mult () , self . delay () , self . dmacfg () , self . mdma () , self . ckmode () , self . vrefen () , self . ch18sel () , self . ch17sel ()) } } # [doc = "ADC common regular data register for dual and triple modes"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
...
impl defmt :: Format for Cdr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Cdr {{ rdata_mst: {=u16:?}, rdata_slv: {=u16:?} }}" , self . rdata_mst () , self . rdata_slv ()) } } # [doc = "ADC Common status register"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
