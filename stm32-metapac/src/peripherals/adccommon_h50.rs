...
fn default () -> Ccr { Ccr (0) } } impl core :: fmt :: Debug for Ccr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Ccr") . field ("ckmode" , & self . ckmode ()) . field ("presc" , & self . presc ()) . field ("vrefen" , & self . vrefen ()) . field ("tsen" , & self . tsen ()) . field ("vbaten" , & self . vbaten ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Ccr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Ccr {{ ckmode: {:?}, presc: {:?}, vrefen: {=bool:?}, tsen: {=bool:?}, vbaten: {=bool:?} }}" , self . ckmode () , self . presc () , self . vrefen () , self . tsen () , self . vbaten ()) } } # [doc = "hardware configuration register"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
