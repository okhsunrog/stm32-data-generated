...
fn default () -> Cr { Cr (0) } } impl core :: fmt :: Debug for Cr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Cr") . field ("rngen" , & self . rngen ()) . field ("ie" , & self . ie ()) . field ("ced" , & self . ced ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Cr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Cr {{ rngen: {=bool:?}, ie: {=bool:?}, ced: {=bool:?} }}" , self . rngen () , self . ie () , self . ced ()) } } # [doc = "status register"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
...
impl defmt :: Format for Sr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Sr {{ drdy: {=bool:?}, cecs: {=bool:?}, secs: {=bool:?}, ceis: {=bool:?}, seis: {=bool:?} }}" , self . drdy () , self . cecs () , self . secs () , self . ceis () , self . seis ()) } } }