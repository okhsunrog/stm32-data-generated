...
fn default () -> Cr2 { Cr2 (0) } } impl core :: fmt :: Debug for Cr2 { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Cr2") . field ("hsi14on" , & self . hsi14on ()) . field ("hsi14rdy" , & self . hsi14rdy ()) . field ("hsi14dis" , & self . hsi14dis ()) . field ("hsi14trim" , & self . hsi14trim ()) . field ("hsi14cal" , & self . hsi14cal ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Cr2 { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Cr2 {{ hsi14on: {=bool:?}, hsi14rdy: {=bool:?}, hsi14dis: {=bool:?}, hsi14trim: {=u8:?}, hsi14cal: {=u8:?} }}" , self . hsi14on () , self . hsi14rdy () , self . hsi14dis () , self . hsi14trim () , self . hsi14cal ()) } } # [doc = "Control/status register (RCC_CSR)"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
