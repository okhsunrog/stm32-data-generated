...
fn default () -> Cr { Cr (0) } } impl core :: fmt :: Debug for Cr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Cr") . field ("lpsdsr" , & self . lpsdsr ()) . field ("pdds" , & self . pdds ()) . field ("cwuf" , & self . cwuf ()) . field ("csbf" , & self . csbf ()) . field ("pvde" , & self . pvde ()) . field ("pls" , & self . pls ()) . field ("dbp" , & self . dbp ()) . field ("ulp" , & self . ulp ()) . field ("fwu" , & self . fwu ()) . field ("vos" , & self . vos ()) . field ("lprun" , & self . lprun ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Cr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Cr {{ lpsdsr: {=bool:?}, pdds: {:?}, cwuf: {=bool:?}, csbf: {=bool:?}, pvde: {=bool:?}, pls: {=u8:?}, dbp: {=bool:?}, ulp: {=bool:?}, fwu: {=bool:?}, vos: {:?}, lprun: {=bool:?} }}" , self . lpsdsr () , self . pdds () , self . cwuf () , self . csbf () , self . pvde () , self . pls () , self . dbp () , self . ulp () , self . fwu () , self . vos () , self . lprun ()) } } # [doc = "power control/status register"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
...
fn default () -> Csr { Csr (0) } } impl core :: fmt :: Debug for Csr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Csr") . field ("wuf" , & self . wuf ()) . field ("sbf" , & self . sbf ()) . field ("pvdo" , & self . pvdo ()) . field ("vrefintrdyf" , & self . vrefintrdyf ()) . field ("vosf" , & self . vosf ()) . field ("reglpf" , & self . reglpf ()) . field ("ewup[0]" , & self . ewup (0usize)) . field ("ewup[1]" , & self . ewup (1usize)) . field ("ewup[2]" , & self . ewup (2usize)) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Csr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Csr {{ wuf: {=bool:?}, sbf: {=bool:?}, pvdo: {=bool:?}, vrefintrdyf: {=bool:?}, vosf: {=bool:?}, reglpf: {=bool:?}, ewup[0]: {=bool:?}, ewup[1]: {=bool:?}, ewup[2]: {=bool:?} }}" , self . wuf () , self . sbf () , self . pvdo () , self . vrefintrdyf () , self . vosf () , self . reglpf () , self . ewup (0usize) , self . ewup (1usize) , self . ewup (2usize)) } } } pub mod vals { # [repr (u8)]
