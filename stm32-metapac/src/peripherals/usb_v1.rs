...
fn default () -> Cntr { Cntr (0) } } impl core :: fmt :: Debug for Cntr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Cntr") . field ("fres" , & self . fres ()) . field ("pdwn" , & self . pdwn ()) . field ("lpmode" , & self . lpmode ()) . field ("fsusp" , & self . fsusp ()) . field ("resume" , & self . resume ()) . field ("esofm" , & self . esofm ()) . field ("sofm" , & self . sofm ()) . field ("resetm" , & self . resetm ()) . field ("suspm" , & self . suspm ()) . field ("wkupm" , & self . wkupm ()) . field ("errm" , & self . errm ()) . field ("pmaovrm" , & self . pmaovrm ()) . field ("ctrm" , & self . ctrm ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Cntr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Cntr {{ fres: {=bool:?}, pdwn: {=bool:?}, lpmode: {=bool:?}, fsusp: {=bool:?}, resume: {=bool:?}, esofm: {=bool:?}, sofm: {=bool:?}, resetm: {=bool:?}, suspm: {=bool:?}, wkupm: {=bool:?}, errm: {=bool:?}, pmaovrm: {=bool:?}, ctrm: {=bool:?} }}" , self . fres () , self . pdwn () , self . lpmode () , self . fsusp () , self . resume () , self . esofm () , self . sofm () , self . resetm () , self . suspm () , self . wkupm () , self . errm () , self . pmaovrm () , self . ctrm ()) } } # [doc = "device address"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
...
fn default () -> Istr { Istr (0) } } impl core :: fmt :: Debug for Istr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Istr") . field ("ep_id" , & self . ep_id ()) . field ("dir" , & self . dir ()) . field ("esof" , & self . esof ()) . field ("sof" , & self . sof ()) . field ("reset" , & self . reset ()) . field ("susp" , & self . susp ()) . field ("wkup" , & self . wkup ()) . field ("err" , & self . err ()) . field ("pmaovr" , & self . pmaovr ()) . field ("ctr" , & self . ctr ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Istr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Istr {{ ep_id: {=u8:?}, dir: {:?}, esof: {=bool:?}, sof: {=bool:?}, reset: {=bool:?}, susp: {=bool:?}, wkup: {=bool:?}, err: {=bool:?}, pmaovr: {=bool:?}, ctr: {=bool:?} }}" , self . ep_id () , self . dir () , self . esof () , self . sof () , self . reset () , self . susp () , self . wkup () , self . err () , self . pmaovr () , self . ctr ()) } } } pub mod vals { # [repr (u8)]
