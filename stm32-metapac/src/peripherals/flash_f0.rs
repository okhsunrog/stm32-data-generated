...
fn default () -> Acr { Acr (0) } } impl core :: fmt :: Debug for Acr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Acr") . field ("latency" , & self . latency ()) . field ("prftbe" , & self . prftbe ()) . field ("prftbs" , & self . prftbs ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Acr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Acr {{ latency: {:?}, prftbe: {=bool:?}, prftbs: {=bool:?} }}" , self . latency () , self . prftbe () , self . prftbs ()) } } # [doc = "Flash address register"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
...
impl defmt :: Format for Cr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Cr {{ pg: {=bool:?}, per: {=bool:?}, mer: {=bool:?}, optpg: {=bool:?}, opter: {=bool:?}, strt: {=bool:?}, lock: {=bool:?}, optwre: {=bool:?}, errie: {=bool:?}, eopie: {=bool:?}, force_optload: {=bool:?} }}" , self . pg () , self . per () , self . mer () , self . optpg () , self . opter () , self . strt () , self . lock () , self . optwre () , self . errie () , self . eopie () , self . force_optload ()) } } # [doc = "Option byte register"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
...
fn default () -> Obr { Obr (0) } } impl core :: fmt :: Debug for Obr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Obr") . field ("opterr" , & self . opterr ()) . field ("rdprt" , & self . rdprt ()) . field ("wdg_sw" , & self . wdg_sw ()) . field ("n_rst_stop" , & self . n_rst_stop ()) . field ("n_rst_stdby" , & self . n_rst_stdby ()) . field ("n_boot0" , & self . n_boot0 ()) . field ("n_boot1" , & self . n_boot1 ()) . field ("vdda_monitor" , & self . vdda_monitor ()) . field ("ram_parity_check" , & self . ram_parity_check ()) . field ("boot_sel" , & self . boot_sel ()) . field ("data0" , & self . data0 ()) . field ("data1" , & self . data1 ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Obr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Obr {{ opterr: {=bool:?}, rdprt: {:?}, wdg_sw: {:?}, n_rst_stop: {:?}, n_rst_stdby: {:?}, n_boot0: {=bool:?}, n_boot1: {=bool:?}, vdda_monitor: {=bool:?}, ram_parity_check: {:?}, boot_sel: {:?}, data0: {=u8:?}, data1: {=u8:?} }}" , self . opterr () , self . rdprt () , self . wdg_sw () , self . n_rst_stop () , self . n_rst_stdby () , self . n_boot0 () , self . n_boot1 () , self . vdda_monitor () , self . ram_parity_check () , self . boot_sel () , self . data0 () , self . data1 ()) } } # [doc = "Flash status register"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
...
fn default () -> Sr { Sr (0) } } impl core :: fmt :: Debug for Sr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Sr") . field ("bsy" , & self . bsy ()) . field ("pgerr" , & self . pgerr ()) . field ("wrprt" , & self . wrprt ()) . field ("eop" , & self . eop ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Sr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Sr {{ bsy: {=bool:?}, pgerr: {=bool:?}, wrprt: {=bool:?}, eop: {=bool:?} }}" , self . bsy () , self . pgerr () , self . wrprt () , self . eop ()) } } # [doc = "Write protection register"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
...
N_BOOT0 = 0x0 , # [doc = "BOOT0 signal is defined by BOOT0 pin value (legacy mode)"]
