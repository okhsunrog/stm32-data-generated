...
# [inline (always)]
pub const fn csr (self) -> crate :: common :: Reg < regs :: Csr , crate :: common :: RW > { unsafe { crate :: common :: Reg :: from_ptr (self . ptr . add (0x0usize) as _) } } } pub mod regs { # [doc = "Control/status register"]
# [repr (transparent)]
# [derive (Copy , Clone , Eq , PartialEq)]
...
# [inline (always)]
pub fn set_tcm_en (& mut self , val : bool) { self . 0 = (self . 0 & ! (0x01 << 7usize)) | (((val as u32) & 0x01) << 7usize) ; } # [doc = "Inverting input secondary selection"]
# [inline (always)]
pub const fn vms_sel (& self) -> super :: vals :: VmsSel { let val = (self . 0 >> 8usize) & 0x01 ; super :: vals :: VmsSel :: from_bits (val as u8) } # [doc = "Inverting input secondary selection"]
# [inline (always)]
pub fn set_vms_sel (& mut self , val : super :: vals :: VmsSel) { self . 0 = (self . 0 & ! (0x01 << 8usize)) | (((val . to_bits () as u32) & 0x01) << 8usize) ; } # [doc = "Non-inverting input secondary selection"]
# [inline (always)]
pub const fn vps_sel (& self) -> super :: vals :: VpsSel { let val = (self . 0 >> 9usize) & 0x03 ; super :: vals :: VpsSel :: from_bits (val as u8) } # [doc = "Non-inverting input secondary selection"]
# [inline (always)]
...
# [inline (always)]
pub fn set_usertrim (& mut self , val : bool) { self . 0 = (self . 0 & ! (0x01 << 18usize)) | (((val as u32) & 0x01) << 18usize) ; } # [doc = "Offset trimming value (PMOS)"]
# [inline (always)]
...
# [inline (always)]
pub fn set_calout (& mut self , val : bool) { self . 0 = (self . 0 & ! (0x01 << 30usize)) | (((val as u32) & 0x01) << 30usize) ; } # [doc = "OPAMP lock"]
# [inline (always)]
...
fn default () -> Csr { Csr (0) } } impl core :: fmt :: Debug for Csr { fn fmt (& self , f : & mut core :: fmt :: Formatter) -> core :: fmt :: Result { f . debug_struct ("Csr") . field ("opampen" , & self . opampen ()) . field ("force_vp" , & self . force_vp ()) . field ("vp_sel" , & self . vp_sel ()) . field ("vm_sel" , & self . vm_sel ()) . field ("tcm_en" , & self . tcm_en ()) . field ("vms_sel" , & self . vms_sel ()) . field ("vps_sel" , & self . vps_sel ()) . field ("calon" , & self . calon ()) . field ("calsel" , & self . calsel ()) . field ("pga_gain" , & self . pga_gain ()) . field ("usertrim" , & self . usertrim ()) . field ("trimoffsetp" , & self . trimoffsetp ()) . field ("trimoffsetn" , & self . trimoffsetn ()) . field ("tstref" , & self . tstref ()) . field ("calout" , & self . calout ()) . field ("lock" , & self . lock ()) . finish () } } # [cfg (feature = "defmt")]
impl defmt :: Format for Csr { fn format (& self , f : defmt :: Formatter) { defmt :: write ! (f , "Csr {{ opampen: {=bool:?}, force_vp: {=bool:?}, vp_sel: {:?}, vm_sel: {:?}, tcm_en: {=bool:?}, vms_sel: {:?}, vps_sel: {:?}, calon: {=bool:?}, calsel: {:?}, pga_gain: {:?}, usertrim: {=bool:?}, trimoffsetp: {=u8:?}, trimoffsetn: {=u8:?}, tstref: {=bool:?}, calout: {=bool:?}, lock: {=bool:?} }}" , self . opampen () , self . force_vp () , self . vp_sel () , self . vm_sel () , self . tcm_en () , self . vms_sel () , self . vps_sel () , self . calon () , self . calsel () , self . pga_gain () , self . usertrim () , self . trimoffsetp () , self . trimoffsetn () , self . tstref () , self . calout () , self . lock ()) } } } pub mod vals { # [repr (u8)]
